      ...
      <div className="flex-1 overflow-y-auto px-6 pt-40 pb-24 relative z-10">
        ...
        {units.length === 0 ? (
          ...
          <div className="space-y-4">
            {units.map(unit => (
              <div key={unit.id} className="bg-white p-6 rounded-[2.5rem] border border-slate-100 shadow-md space-y-4 animate-slide-up">
                <div className="flex justify-between items-start">
                  ...
                  <div className="flex space-x-2">
                    <button onClick={() => handleDeleteUnit(unit.id)} className="p-2 text-red-400 hover:bg-red-50 rounded-lg active:scale-90 transition-all">
                    ...
                    {unit.status === 'draft' && (
                      <button onClick={() => handlePublish(unit.id)} className="px-3 py-1 bg-blue-600 text-white rounded-lg text-[9px] font-black uppercase tracking-widest active:scale-95 transition-all">
                        Publish
                      </button>
                    )}
                  </div>
