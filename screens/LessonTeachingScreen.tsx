  ...
  const handleReadAloud = async (id: number | string, textToRead: string) => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        ...
        contents: [{ parts: [{ text: `Read clearly: ${primaryText}` }] }],
        config: {
        ...
        callbacks: {
          ...
          onmessage: async (message: LiveServerMessage) => {
            ...
            if (message.serverContent?.turnComplete) {
              if (currentTurnRef.current.user || currentTurnRef.current.ai) {
                sessionTranscriptsRef.current.push({ ...currentTurnRef.current });
              }
